...
class Futures extends FunSuite with ScalaFutures with Matchers with TypeCheckedTripleEquals {
  ...
  test("Check that the future is ready within 500 milliseconds (or not)") {
    def check(f: Future[Int]): Boolean = f.???(500 millis)
  ...
  test("Race between two futures (i.e keep the fastest one)") {
    ...
    val faster = Future.???(Seq(slow, fast))
  ...
  test("Run a list of futures") {
    ...
    assert(Future.???(futs2).futureValue === (1 to 50).map(_.toString)) // Wait for them in parallel
  }
  ...
  test("Run the same future constructor over a list of values (we need to 'traverse' a sequence of Int))") {
    def f(n: Int) = fut(n + 1)
    assert(Future.???(1 to 50)(f).futureValue === (2 to 51)) // Create them and wait for them in parallel
  }
